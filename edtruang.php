...
<julin>
	...
	<?php
	 $no = $_GET['no'];
	 $ndelok = "SELECT * FROM ruang WHERE no = $no";
	 $query = mysqli_query($login, $ndelok);
	 $jadi = mysqli_fetch_array($query);

	 $no = $jadi['no'];
	 $code = $jadi['code'];
	 $nm_ruang = $jadi['nm_ruang'];
	?>
		<form method="POST" action="php/edt-ruang.php?no=<?php echo $jadi['no'];?>">
			<div class="form-group">
			 <label>Ruang</label>
			 <input type="text" value="<?php echo $jadi['code'];?>" class="form-control setengah" name="code" placeholder="Code" maxlength="6" required>
			 <input type="text" value="<?php echo $jadi['nm_ruang'];?>" class="form-control setengah" name="nm_ruang" placeholder="Ruang" maxlength="60" required>
