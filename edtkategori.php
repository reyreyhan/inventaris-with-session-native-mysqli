...
<julin>
	...
	<?php
	 $no = $_GET['no'];
	 $ndelok = "SELECT * FROM kategori WHERE no = $no";
	 $query = mysqli_query($login, $ndelok);
	 $jadi = mysqli_fetch_array($query);

	 $no = $jadi['no'];
	 $code = $jadi['code'];
	 $nm_kategori = $jadi['nm_kategori'];
	?>

		<form method="POST" action="php/edt-kategori.php?no=<?php echo $jadi['no'];?>">
			<div class="form-group">
			 ...
			 <input type="text" value="<?php echo $jadi['nm_kategori'];?>" class="form-control setengah" name="nm_kategori" placeholder="Kategori Barang" maxlength="60" required>
